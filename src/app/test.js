    ...
    .controller('PortStatusReportController', function ($scope, $rootScope, fetchResult, $stateParams, ip, ngSocket, $location, $timeout, $window) {
        ...
        $scope.getDslamDetaile = function () {
            ...
            fetchResult.fetch_result({
                ...
                $scope.dslamType = result.data.dslam_info.dslam_type_info.name;
                // if( $scope.dslamType == 'fiberhomeAN2200')
                // {
                // $scope.IsFibeHome = true;

                //	}
                //else{
                // console.log('dslamType ' ,'NOOOOOOOOOOOOOOOOO');
        ...
        $scope.run_command = function () {
            ...
            if ($scope.dslamType != 'fiberhomeAN2200' && $scope.dslamType != 'fiberhomeAN3300' && $scope.dslamType != 'fiberhomeAN5006') {
                if ($scope.command != '') {
                    ...
                    if ($scope.command == 'change admin status') {
                        ...
                        if ($scope.selected_command_type.line_profile != undefined && $scope.selected_command_type.line_profile != null) {
                            $scope.run_command_loader = true;
                            $scope.new_lineprofile = $scope.selected_command_type.line_profile.name
                            var params = {
                                "type": "dslamport",
                                "is_queue": false,
                                'dslam_id': $scope.dslam_id,
                    ...
                    fetchResult.fetch_result({
                        ...
                        data: {
                            ...
                            "new_lineprofile": $scope.selected_command_type.line_profile ? $scope.selected_command_type.line_profile.name : '',
                ...
                fetchResult.fetch_result({
                    ...
                    url: ip + 'api/v1/dslamport/command2/'
                }).then(function (result) {
                    $scope.res2 = result
                    $scope.IsFiberHome = true;
                    $scope.getCommandResult();
                    console.log(result);
